...
- **Verified type-once submit model** - the digest is typed once (`send-keys -l`
  ...
  For Herdr's non-Claude idle-baseline path it means native agent-state observed a turn start, the shared classifier proved the composer cleared, or the shared queued-Enter verdict proved delivery while busy; a known Claude target uses the current rendered-footer idle-to-busy transition or a cleared composer and never treats native `working` alone as delivery.
