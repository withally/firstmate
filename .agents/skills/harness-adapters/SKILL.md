...
Agent lifecycle mechanics are owned by the executable control plane in `bin/fm-control-lib.sh` and delivered by `bin/fm-control.sh <task-id> interrupt|exit|relaunch`; see [`docs/agent-control.md`](../../../docs/agent-control.md) for the contract.
...
Codex CLI 0.150.1 accepts the top-level boolean override `check_for_update_on_startup=false` under strict config, and the `bin/fm-spawn.sh` Codex launch template passes it with `-c` so an interactive update modal cannot block an unattended brief.
...
Regression coverage is `tests/fm-tmux-submit-busy.test.sh`, `tests/fm-composer-lib.test.sh`, and `tests/fm-backend-herdr.test.sh`; the live Herdr submit-confirmation guard is `FM_HERDR_SUBMIT_CONFIRM_LIVE=1 tests/fm-herdr-submit-confirm-live-e2e.test.sh`, and the away-mode Claude guard is `FM_AFK_HERDR_CLAUDE_LIVE=1 tests/fm-afk-herdr-claude-busy-guard-live-e2e.test.sh`.
...
| Interrupt | Single Escape, which closes the run with `terminal: cancelled` AND restores the interrupted prompt into the composer as real bright text, so `fm-control` follows Escape with `C-u` to clear it; `fm-send`'s legacy key path reads the same interrupt-follow-up clear table. |
