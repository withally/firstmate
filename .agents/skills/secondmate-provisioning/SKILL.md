...
It writes the durable `.fm-secondmate-parent` return-route binding before the required `.fm-secondmate-home` identity marker; both are gitignored and must remain in place for inactive-terminal reporting and home validation.
The seed helpers and `bin/fm-secondmate-parent-lib.sh` own their exact route schema and refusal mechanics.
...
`config/trace-context` is in the same declared set but is session-scoped: it converges only at the launch point, where the primary also hands the new process its frozen decision, and live convergence into an already-running home reports it `unchanged` instead of rewriting a decision that home has already frozen ([`docs/trace-context.md`](../../../docs/trace-context.md)).
