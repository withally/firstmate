...
It writes the gitignored `.fm-secondmate-parent` durable binding before the required `.fm-secondmate-home` identity marker, `bin/fm-teardown.sh` removes that binding before a retired local home returns to the pool, ordinary spawn refuses a leftover foreign binding, and every local upward consumer refuses a child absent from the claimed parent's `data/secondmates.md`; the parser header in [`bin/fm-secondmate-parent-lib.sh`](../../../bin/fm-secondmate-parent-lib.sh) owns the record contract.
