...
It writes the gitignored `.fm-secondmate-parent` durable binding before the required `.fm-secondmate-home` identity marker, `bin/fm-teardown.sh` removes that binding before a retired local home returns to the pool, ordinary spawn refuses a malformed or unproven foreign binding and clears a firstmate-shaped local binding only when its recorded parent is not the current home and is absent or no longer registers the worktree, and every local upward consumer refuses a child absent from the claimed parent's `data/secondmates.md`; the parser header in [`bin/fm-secondmate-parent-lib.sh`](../../../bin/fm-secondmate-parent-lib.sh) owns the record contract.
