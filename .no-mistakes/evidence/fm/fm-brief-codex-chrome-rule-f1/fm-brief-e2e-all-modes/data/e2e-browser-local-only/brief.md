...
1. First action: create your branch: `git checkout -b fm/e2e-browser-local-only`

# Rules
1. Never push to any remote and never open a PR. Work only on your `fm/e2e-browser-local-only` branch; firstmate handles the merge into local `main`.
...
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/Users/ivan/.no-mistakes/evidence/01M1XBMF7J1M4CXHMPEMXSR1G6/fm-brief-e2e-all-modes/state/e2e-browser-local-only.status'`
...
The task is complete only when committed on your branch `fm/e2e-browser-local-only`. Do NOT push, do NOT open a PR, do NOT merge.
Keep your branch a clean fast-forward onto the current default branch - if `main` has advanced, rebase onto it so the eventual merge stays a fast-forward.
When it is implemented and committed, append `done: ready in branch fm/e2e-browser-local-only` to the status file and stop.
