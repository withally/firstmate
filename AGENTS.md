...
state/               runtime records and signals; gitignored
  ...
  branch-outcomes.jsonl branch-action/ .branch-outcomes-cursor  Pi supervision-branch durable outcome store, firstmate-action handoff markers, and read cursor; bin/fm-branch-outcome.sh owns the format
  ...
  .<id>.signal-open-keys  attended signal classifier's per-task open-key snapshot at its signal byte boundary; owned only by signal triage, never the drain cursor, and retired with the task
...
3. **Wake queue** - when locked, presents the durable wake queue and prints the raw records prominently as this turn's first work queue; a clearly labeled status-event annotation may follow a valid `signal` record and includes every status line still unread at the presentation cursor, but never replaces the raw record or current-state reconciliation, and a lapsed watcher chain still surfaces here via the same guard alarm.
   ...
   The same drain prints every still-unread nonterminal status line since the last presentation in an unbounded `UNREAD STATUS` section, except a keyed `resolved` that closes an open decision because its signal annotation is the single presentation; those lines are not re-printed after that presentation.
...
Do not reconstruct or supervise a secondmate's child tree from the main home; the bounded Pi-primary terminal-status mirror is owned by [docs/pi-supervision-branch.md](docs/pi-supervision-branch.md#two-stage-noise-filter).
...
When a steer answers an open keyed decision or blocker, pass `fm-send`'s `--resolve-key` so the answer itself closes that decision record at answer time; Pi-mirrored secondmate child decisions follow the mate-home path in [docs/pi-supervision-branch.md](docs/pi-supervision-branch.md#two-stage-noise-filter) (contract: `bin/fm-send.sh` header).
...
A captain instruction to merge is explicit authority; the `self` tier is standing routine merge authority, and the `firstmate` tier requires its keyed parent approval record.
