...
config/calm          Pi Calm presentation preference written by the optional `/calm` extension; LOCAL, gitignored; only "on" or "off", and absent or unrecognized = off; home-local and NOT inherited into secondmate homes (docs/calm.md)
...
A crewmate that declares a deliberate external wait with a `paused:` status is the one other absorb case: its idle pane is expected, so the watcher absorbs it like a working crew but rechecks it only on the long `FM_PAUSE_RESURFACE_SECS` cadence (default 3600s, doubling on unchanged repeats up to `FM_PAUSE_RESURFACE_MAX_SECS`, default 14400s) rather than the wedge threshold, re-surfacing a `paused ... recheck` wake once per window so a forgotten pause cannot rot invisibly - unlike `blocked:`, which stays a genuine stuck-crew escalation.
