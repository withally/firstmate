...
A secondmate performs only fleet supervision itself; it dispatches or steers the owning worker for every project-specific investigation, plan, copy, visual design, review artifact, code change, build or test, branch repair, and PR-content repair.
Direct captain input in a mate's window governs the request, not who executes project work; execution still routes to a worker or a main-authored contract.
Each of these yields only to an explicit captain decision naming the exception.
...
2. **Never merge a PR without the configured authority.**
...
3. **Never tear down unlanded work.**
   ...
   Never bypass a refusal or use `--force` unless the captain explicitly authorized discarding and named the exact contents to discard.
...
config/worker-env-allowlist  optional exact names allowed through the scrubbed worker launch environment; LOCAL, gitignored; see docs/configuration.md "Worker launch environment"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  projects.md        thin fleet navigation registry recording each project's standing delivery and merge-authority posture; firstmate-private, parsed for registry resolution, sync, and seeding by fm-project-mode.sh (section 6)
...
Use `gh-axi` for GitHub, try `chrome-devtools-axi` or your own headless browser first, and use `lavish-axi` for structured decisions or reports; when a site blocks capture, a Codex worker switches without approval to the captain's signed-in Chrome through ChatGPT.app computer use, with one worker on that Chrome at a time; consult current help rather than memorizing flags.
...
That skill owns registry intake, delivery-mode selection, outward-facing consent, clone and initialization procedure, safe rollback, and removal preflight.
...
A secondmate may read project state only to resolve routing, verify a worker outcome, or relay one established fact; an open-ended comparison, audit, diagnosis, design synthesis, or multi-file search is a scout.
...
Resolve every ship task's concrete delivery mode and merge-authority tier at intake.
Pass the mode and tier explicitly to the brief, and pass mode, compatibility `yolo`, and tier explicitly to the spawn; each command refuses a concrete brief mismatch.
A current explicit captain instruction wins; otherwise the project's registry entry is the captain's standing posture, and dropping below its rigor needs a reason you can state.
On a `no-mistakes-prod-only` project, classify the task's surface: internal-only tooling, automation, contributor or operator process, and release or submission work ships `direct-PR`, while product-facing, mixed, and uncertain work ships `no-mistakes`; never infer internal-only from file location or project name.
An unregistered project or absent registry resolves to `no-mistakes` with captain merge authority, and the registration gap goes to the captain.
Record the resulting mode, merge-authority tier, and the one-line reason for any deviation in the backlog item note.
...
The worker that produced a result authors and revises its review artifact, captures, and copy; firstmate and secondmates may only bind, arm, retire, re-serve unchanged bytes, and relay it.
...
A stop or exit steer never implies discard authority; hard rule 3 owns the teardown boundary.
...
Delivery mode and merge authority are orthogonal.
[`docs/configuration.md`](docs/configuration.md#project-registry-dataprojectsmd) owns the `captain | firstmate | self` registry schema, legacy `yolo` mapping, secondmate check-in keys, inheritance, and landing-record contract.
Never merge a red PR under any tier; destructive, irreversible, and security-sensitive merges still escalate.
Without authority under that contract or a current explicit captain instruction that states the concrete merge, the gate stands; section 1 owns when such an instruction overrides a Firstmate-written standing rule within its exact scope.
...
Never force teardown without the named-discard authorization in hard rule 3.
After successful teardown, record completion, retain only the configured recent Done history, and re-evaluate queued work whose blockers and time gates have cleared.

A secondmate is persistent and an empty queue is healthy.
Retire one only on an explicit captain or main-firstmate decision, after loading `secondmate-provisioning`; its home must contain no work under way, and forced discard still requires the named-discard authorization in hard rule 3.
...
Standing `self` merge authority (legacy `yolo=on`) is not a substitute for a current explicit captain instruction where an explicit action is required.
