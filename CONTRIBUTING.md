...
tests/fm-daemon.test.sh                   # sub-supervisor classifier, declared-external-wait rechecks, away-mode compatibility preflight refusing Pi + Herdr with no flag/daemon/backend side effects, /afk presence-gating, fm-afk-start daemon-lock lifecycle, max-defer, composer, and fm-send submit tests
...
tests/fm-brief.test.sh                    # fm-brief.sh bash -n parse regression guard, clean delivery-mode generation, and configured-pause scaffolds
