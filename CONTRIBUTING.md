...
The PR that first introduces this refresh may still show the required check red after the ten-minute bound while all real checks are green because its PR body still carries the preceding pipeline attestation; treat that as the known self-hosting condition and do not start another repair round.
