...
- **Explicit project modes and merge authority** - each project ships via `no-mistakes`, `direct-PR`, or `local-only` under a configured merge-authority tier; see the [project registry configuration](docs/configuration.md#project-registry-dataprojectsmd).
...
- [docs/gsd-method.md](docs/gsd-method.md) - the GSD research-and-plan method as Firstmate runs it without the tool: kept rules, enforcing mechanisms, model seats, and its dated log.
