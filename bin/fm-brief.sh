...
# Every scaffold also carries the shared operational-input silence rule once
# under that status protocol; bin/fm-operational-input.sh owns its wording.
...
When a message carries that marker, perform only fleet supervision yourself: dispatch any project-specific investigation, planning, audit, copy, visual design, review-artifact authorship, code, build, test, branch repair, or PR-content repair request to the owning worker or scout, then relay the child result via the STATUS/ESCALATION path below, never only in this chat: the main firstmate does not read your chat, so a chat-only reply is lost.
