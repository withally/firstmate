...
FM_DELIVERY_CLAUDE_BUSY_REGEX_DEFAULT='^[[:space:]]*(✢|✽|✻|✶|✳|•)[[:space:]]+[[:alpha:]][[:alpha:][:space:]]*…[[:space:]]+\([0-9]+[smh][^)]*\)[[:space:]]*$|^[[:space:]]*(✢|✽|✻|✶|✳|•)[[:space:]]+[[:alpha:]][[:alpha:][:space:]]*\([0-9]+[smh][[:space:]]+[·•][[:space:]]+[^)]*\)[[:space:]]*$|^[[:space:]]*esc to interrupt[[:space:]]*$'
