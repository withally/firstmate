...
launch_template() {
  ...
  case "$harness" in
    ...
    codex)
      # Disable Codex's per-startup update check so an interactive update modal
      # cannot block the unattended worker before it processes the brief.
      if [ "$kind" = secondmate ]; then
        printf '%s' 'codex __MODELFLAG____EFFORTFLAG__--dangerously-bypass-approvals-and-sandbox -c check_for_update_on_startup=false "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
      else
        printf '%s' 'codex __MODELFLAG____EFFORTFLAG__--dangerously-bypass-approvals-and-sandbox -c check_for_update_on_startup=false -c "notify=[\"bash\",\"-c\",\"touch __TURNEND__\"]" "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
      fi
      ;;
