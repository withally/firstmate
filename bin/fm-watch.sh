...
EOF
    ...
    [ "$beat_age" -gt "$threshold" ] || [ "$age" -gt "$very_old_age" ] || continue
    row_key="$epoch-$seq"
...
while :; do
  ...
  # Observe the foreign queue and its watcher's beat before the rest of this
  # cycle so a genuinely stalled or very old row wakes the parent without
  # consuming or rewriting the receiving home's record.
  secondmate_wake_stall_tick || {
