...
A stale recovery generation changes neither artifact; if the generation changes after receipt closure, the closed receipt and still-queued wake remain, and a later valid generation idempotently retires the queue row, so interruption can leave both but never neither.
