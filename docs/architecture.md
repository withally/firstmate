...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, check-script output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past the current pause re-surface cadence (`FM_PAUSE_RESURFACE_SECS`, doubling on unchanged repeats up to `FM_PAUSE_RESURFACE_MAX_SECS`), and heartbeat backstop hits.
...
That block owns the live wait shape for the running primary harness: Claude and Grok use background-notify cycles, Codex uses bounded foreground checkpoints, Pi uses its two tracked supervision extensions, and OpenCode uses its TUI plugin.
...
`fm-send.sh` selects a pre-Enter popup-settle for slash commands and for codex `$...` skill invocations using metadata-routed target `harness=` values, gives long messages (240+ characters) the same longer pre-Enter settle plus extra Enter-only retries (unless the caller set `FM_SEND_RETRIES`) so a busy pane's slow multi-line composer render cannot swallow the submit, then adds its own `FM_SEND_SETTLE` pause after successful text sends so immediate peeks catch the receiving turn starting; the sub-supervisor uses only the shared submit core and does not pay that post-submit pause.
