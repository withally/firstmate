...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, the built-in fseventsd footprint early warning, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Generated secondmate charters reject generic receipt or start acknowledgements, key only supervisor-actionable material phase reports, require project-output status lines to name the child task and outcome receipt, and close an opened phase with a same-key later state or `resolved` event, while the structured home remains authoritative even if that closure is missing.
...
Composer classification has one shared owner, `bin/fm-composer-lib.sh`: tmux, herdr, Zellij, Orca, and cmux contribute only screen captures plus declarative styled, cursor, identity, and row capabilities.
Herdr additionally supplies the bounded settle proof for its cursorless Pi fallback, while the shared classifier owns every shape and the `empty`/`pending`/`pending-unproven`/`unknown` verdict.
...
A direct captain message governs the request but does not authorize the mate to execute project work; the charter routes execution to the owning worker or a main-authored contract.
...
Idle secondmate panes are healthy; teardown is explicit and refuses while the secondmate home has in-flight work unless the captain authorizes the named contents for discard under hard rule 3 with `--force`.
