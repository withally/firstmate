...
All three read that one predicate from `bin/fm-wake-lib.sh`, so they cannot disagree: a live daemon whose watcher beacon has gone stale is reported as degraded rather than as no supervision at all, while a missing daemon keeps the full alarm.
