...
Actionable wakes include captain-relevant status signals, status signals whose shape is not a recognized routine nonterminal one, recognized routine nonterminal signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, the built-in fseventsd footprint early warning, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
In attended mode, only positively recognized nonterminal lifecycle signals (the progress and declared-wait verbs plus informational `note:` lines) and bare turn-ended signals are candidates for routine absorption, and they are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: an actively running no-mistakes step attributed to that crew's current code, or an exact busy verdict from the semantic busy-state contract.
Unknown syntax, dead or unreadable endpoints, terminal verbs, checks, and stale or wedge evidence continue to wake immediately, while `config/attended-routine-status-absorb` can disable the conservative default-on policy for one home.
The away-mode daemon shares that same shape policy as its syntax boundary, and treats a signal path that teardown already removed as self-handled rather than escalating a digest entry with no content behind it.
...
Its initial normal-mode status signal still surfaces through the routine-nonterminal path, while away mode self-handles that routine signal and owns the later recheck.
...
A repeat presentation of an unchanged set collapses to an `OPEN DECISIONS: unchanged, N open` marker that refers back to the already-presented block, and a session-recovery or mechanical drain never spends that collapse; [`configuration.md`](configuration.md#attended-routine-status-absorption-configattended-routine-status-absorb--fm_attended_routine_status_absorb) owns that boundary.
...
A queued signal annotation prints every status line still unread at that cursor, while the fleet-wide UNREAD STATUS section prints `note:` lines, reserved-key pending-reply resolutions, and attended routine lines previously absorbed by bash exactly once on the next genuine presentation.
An absorbed-status receipt binds the status-file identity and byte endpoint before the watcher advances its signal suppressor; cursor commit retires that receipt only after those bytes are printed, so a crash replays rather than loses the delayed status.
A receipt that can no longer describe the current file, because its identity changed or its endpoint sits past the end, is dropped and read as absent: those bytes went away with the file they described, and the drain's no-loss sections must never be disabled by one unverifiable receipt.
...
The always-on watcher also uses that library's one shared attended absorption policy on routine nonterminal signals, and its absorb classification on first-sighting stale panes before status-log terminality is trusted, while the daemon maintains distinct wedge and declared-wait recheck cadences.
