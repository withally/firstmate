...
In ordinary active-session mode, a status-only signal batch whose every latest event is `working:` is routine progress and is absorbed without a runtime busy proof.
...
If a worker writes `working:` and then stops with no terminal event, the unchanged-pane stale path still surfaces it after three total pane observations, adding at most two `FM_POLL` intervals after the progress signal is classified, or 30 seconds at the defaults.
