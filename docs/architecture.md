...
Once that row reaches `FM_SECONDMATE_WAKE_STALL_SECS`, the primary appends one keyed `check` wake only when the secondmate home's own `state/.last-watcher-beat` is older than the same threshold, or when the row age exceeds three times the threshold regardless of the beat.
