...
Actionable wakes include captain-relevant status signals, authenticated check output such as PR merge polling or a Relay mention, the built-in fseventsd footprint early warning, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Attended signal triage applies the shared captain-relevant predicate to newly appended status lines, absorbing all-nonterminal appends and bare turn-ended signals for ordinary crews and secondmates alike while a keyed `resolved` wakes only when it closes a currently open fold key.
...
A queued signal annotation prints every status line still unread at that cursor, while the fleet-wide UNREAD STATUS section prints nonterminal status lines once even on an empty-queue drain because those events do not belong to the OPEN DECISIONS fold.
