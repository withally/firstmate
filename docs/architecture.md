...
Actionable wakes include captain-relevant status signals, ambiguous lifecycle signals whose crew's agent has confidently exited without a captain-relevant terminal line (a swallowed finish) or whose status batch carries an unknown or unmatched-decision line, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working and whose status log is terminal (captain-relevant) or whose agent has confidently exited, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` and, on re-verification against the authoritative current run state at that boundary, are no longer provably working, declared external waits past an explicitly configured `FM_PAUSE_RESURFACE_SECS` recheck cadence (unset by default), and heartbeat backstop hits.
An attended primary that finds a crew merely gone quiet between steps - not provably working, no declared wait, but its agent still live - does not end its single sitting wait for it; it hands the crew to the stale path's wedge timer, which starts the clock silently and surfaces it as a possible wedge only after `FM_STALE_ESCALATE_SECS`, so a live idle crew never forces a supervision turn while a genuinely stuck one still escalates on schedule.
Away mode is unchanged: its daemon keeps queuing every ambiguous signal for classification rather than applying this quiet-crew absorb.
A stale pane whose terminal status line is byte-identical to its already-delivered `.hb-surfaced` marker is absorbed instead of re-surfacing once per pane-hash change, and a stale pane behind a declared `paused:` or verified `captain-held` status is absorbed regardless of agent liveness.
A provably-working stale is re-verified against the authoritative current run state at each `FM_STALE_ESCALATE_SECS` boundary and keeps absorbing silently while an active run-step still reports it working, so a still-running validation legitimately sitting on a static pane never forces a supervision turn; it escalates only once the crew is no longer provably working (a run that reached a gate, finished, failed, or went unreadable).
Repeated such escalations on the same unchanged pane add an escalation count to the wake reason and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker.
...
After lifecycle and process-event reconciliation, `bin/fm-inactive-reconcile.sh` supplies a separate interval-gated, aggregate-budgeted, cursor-resumable defense-in-depth scan for ordinary direct reports whose wake edge may have been missed.
It reads only durable local state and the existing `bin/fm-crew-state.sh` current-state interface, accepts only confirmed `done` or `failed`, and never invokes a forge, network client, PR poll, or task check script.
Its fingerprint binds the spawn incarnation, task id, terminal verdict, PR text, and sanitized last status independently of heartbeat suppression state.
In a main home the scan creates a pending receipt before queueing `inactive-outcome:`; presentation alone changes nothing, and only the existing post-handling `--ack-through <seq> --recovery-generation <gen>` path closes the receipt before consuming that exact wake.
A moved recovery generation is non-fatal here: the receipt still closes and rows through `--ack-through` are still consumed, while only the newer episode is left pending and the drain prints its re-drain remedy (see [watcher-continuity.md](watcher-continuity.md#recovery-episode-acknowledgement)); an interruption between receipt closure and row consumption leaves the closed receipt and the still-queued wake, and a later acknowledgement idempotently retires the row, so interruption can leave both but never neither.
In a secondmate home the seeded `.fm-secondmate-parent` binding selects the established local parent-status or remote relay-input append path; the exact report is appended idempotently before the receipt becomes reported, while an invalid route retains the receipt and queues a repair wake with a later path to delivery and retirement.
`bin/fm-inactive-reconcile-lib.sh` owns receipt transitions, and `bin/fm-secondmate-parent-lib.sh` owns return-route parsing.
In ordinary active-session mode, an ordinary direct report's status-only signal batch whose every latest event is `working:` is routine progress and is absorbed without a runtime busy proof.
When `fm-send.sh` confirms delivery of an answer carrying exactly one valid `[key=<slug>]` for a decision still open in the target task's status stream, it records a durable private task+key delivery receipt under `state/.delivered-decisions/`.
The first matching keyed `resolved` echo - the key as the prefix token or as the line's exact final note token - is then absorbed and triage-logged, and its receipt is consumed.
Unmatched or bundled `resolved` events, malformed or ambiguous keys, stale receipts, and any batch containing another unabsorbed captain-relevant or unrelated event escalate instead, and a receipt that cannot be cleared re-queues the wake rather than risking a second silent absorb.
A repeatable `fm-send.sh --resolve-key <slug>` additionally closes the decisions an answer resolves at answer time instead of waiting for the worker to echo them: every named key must already be open in this home's target-task ledger or the send refuses before delivering anything, and only after confirmed delivery and any pending-reply commit does `fm-send.sh` append one matching `resolved [key=<slug>]:` event per key.
Failed or unconfirmed delivery closes nothing, and an append that does not clear the key leaves it open and prints the exact manual close action rather than reporting a closed decision.
The option needs a task selector resolved through this home's metadata, so an explicit backend target and a `--key` send both refuse it, and it does not relax the explicit `FM_HOME` requirement.
A single-key answer still records its `.delivered-decisions/` receipt before appending the resolved event, so the watcher absorbs that self-written echo through the same first-matching-echo path; a multi-key answer records no receipt.
A status signal batch is likewise absorbed on the signal path when every status line is captain-relevant and byte-identical to its already-delivered `.hb-surfaced` marker; a first or changed terminal line, a mixed or ambiguous batch, or a coalesced re-delivery that would mask a never-surfaced captain-relevant event still escalates.
On the attended primary a bare turn-ended signal or other ambiguous signal shape surfaces only when the captain would care: `bin/fm-crew-state.sh` no longer reports the crew provably working (no actively running no-mistakes step attributed to that crew's current code and no exact busy verdict from the semantic busy-state contract) and either the status carries a captain-relevant verb, the crew's agent has confidently exited (`fm_backend_agent_state` reports the endpoint dead or missing, a swallowed finish no later event would catch), or the batch carries an unknown or unmatched-decision line.
A crew that is not provably working, declared no wait, and whose agent is still live has merely gone quiet between steps; it is absorbed and left to the wedge timer as above rather than ending the sitting wait, and the heartbeat backstop still re-surfaces any captain-relevant status such an absorb misses.
On the attended primary a persistent secondmate's routine `working:` phase report is absorbed too: `signal_secondmate_routine_working` classifies the batch line by line and absorbs it when every latest event's leading verb is `working` and no latest line carries a pending-reply correlation token (`corr=<16hex>`).
The one secondmate line that must still reach firstmate is a correlated answer to a marked request, which always embeds that corr token, so it and any non-working secondmate line still surface; the stale path deliberately skips an idle secondmate endpoint and the heartbeat backstop only rescans captain-relevant statuses, so no later path would deliver that answer.
Away mode is unchanged and still forces immediate delivery of every secondmate report.
A crew that declares `paused:` for a known external wait is separately absorbed while idle, rather than being treated as a possible wedge.
A durable `captain-held` transfer earns the same bounded absorb, and both declarations hold regardless of agent liveness: the declaration is the crew's own positive idle-by-design statement, and an active run-step or busy pane normally outranks it - except when the backend confirms the agent endpoint dead or missing, which proves any leftover harness busy event is stale, so the paused declaration keeps absorbing.
The secondmate idle-endpoint exemption is unchanged.
The declaration is honored on the signal path as well as the stale path: a `paused:` or verified `captain-held` status append and the bare turn-end under it are already-handled "still waiting" facts and are absorbed in normal mode too (a resumed crew surfaces once it writes a non-declaration status), so a declared captain-wait window raises no monitoring alert, while away mode self-handles the same routine signal and owns any opted-in later recheck.
Fresh stale panes use the same current-state read before trusting the status log, so an active run or a proven busy worker outranks an old captain-relevant status-log line left behind before validation.
No-change heartbeats are also benign.
Absorbed wakes advance their suppression markers, log to `state/.watch-triage.log`, and keep the watcher blocking without a queue record or LLM turn.
After each presentation, `fm-wake-drain.sh` runs the same liveness guard as the supervision scripts, so a lapsed watcher chain surfaces even on a turn that only handles queued wakes.
Routine watcher polling, supervision no-ops, elapsed waiting time, and absorbed benign wakes stay silent.
A declared external wait is fully silent once absorbed - the captain monitors idle windows through Herdr and pull-based digests - unless `FM_PAUSE_RESURFACE_SECS` is explicitly set, which opts back into one bounded recheck per pause window.
...
`fm-wake-drain.sh` prints a separate, fleet-wide OPEN DECISIONS section on every presentation, including the empty-queue path session-start relies on, built from `fm-classify-lib.sh`'s `status_open_decisions` fold so the buried decision keeps surfacing until it is explicitly resolved.
That scan is bound to the drain's already-captured presentation snapshot and runs incrementally: per status file it persists a sibling `.<id>.open-decisions-cursor` recording the byte offset, device/inode identity, file generation, and exact open set, so a warm drain refolds only newly appended bytes while producing the identical open set, and `status_open_decisions` remains the one authoritative whole-history rule it rebuilds from on absent, malformed, truncated, replaced, or identity-reused cursor state.
The same presentation holds a fleet-wide cursor lock, snapshots each status file's identity and byte endpoint, prints every unread line for a direct status signal plus every unread `note:` and pending-reply resolution, and advances `.status-presentation-cursor` only after that complete captured surface succeeds.
A status file's first presentation, before its `.status-presentation-cursor` row exists, seeds its endpoint from a valid existing `.<id>.open-decisions-cursor` offset so a migrating task surfaces only newly appended bytes instead of replaying its whole unread history; a missing or invalid seed cursor fails safe to byte zero.
A later append falls beyond the snapshot and remains unread for the next drain, while teardown retires the exact task's cursor row with its status log so a reused identity begins at byte zero.
This home's answerer close and captain-held transfer use the guarded append in `bin/fm-wake-lib.sh`, which advances the watcher marker only when every earlier byte was already announced and no foreign writer interleaved.
New openings use one canonical `needs-decision [key=<slug>]:` record per line, but the fold also reinterprets the historical shapes already written into ledgers: an opening whose keys arrive as a contiguous run of exact final note tokens, or as one complete `[key=<slug>]` token at the head of the note, folds under those real keys instead of colliding in the shared `default` bucket, and each of those keys stays independently closable by its own `resolved` line.
A key before the colon stays authoritative over any key-like prose, and a malformed slug falls back to `default` rather than hiding the opening, so an unanswered decision always stays visible.
`bin/fm-classify-lib.sh`'s header owns the exact decision-key grammar and the compatibility paths.
...
That block owns the live wait shape for the running primary harness: Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Grok uses a monitor-owned successor-first coordinator, Codex uses bounded foreground checkpoints, Pi and pi-signed use the same two tracked primary extensions, and OpenCode uses its TUI plugin.
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it forks the watcher as a tracked child, verifies it is genuinely alive with a fresh liveness beacon, and prints an honest `started`, `attached`, `owner verified` (a duplicate returning behind the home's one identity-bound arm owner), or nonzero `FAILED` status.
[`watcher-continuity.md`](watcher-continuity.md#arm-layer-cycle-contract) owns the arm layer's successor, clean-close receipt, terminal-delivery, and typed reasonless-death contract.
...
On Pi, guard notifications and the registered `fm_watch_arm_pi` tool prove that the tracked primary extensions loaded; they do not make Calm a delivery filter or change watcher classification.
The watcher extension turns each emitted actionable reason into an ordinary user-role follow-up, and [Calm](calm.md) can hide that row without suppressing its model input or assistant reply, so unnecessary routine turns must be prevented before delivery by keeping routine progress out of the watcher reason stream.
Missing, failed, or unhealthy cycle notifications come from the separate continuity and turn-end-guard path and are not evidence that a routine status signal was misclassified unless correlated runtime evidence connects the two.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if work, process-event sources, away mode, or X-mode relay polling has an unhealthy model-aware supervision verdict, or if queued wakes are waiting to be handled.
The presentation script calls that guard after showing the queue; until handling acknowledges the generation, its queued-wakes warning deliberately remains visible.
That warning is recovery-marker-aware: inside the handling window it asks for the drain's `WAKE_ACK_REQUIRED` acknowledgement instead of instructing the handling turn to drain again.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work, a process-event source, away mode, or X-mode relay polling needs supervision and no identity-matched supervisor is proven live, direct Stop hooks block and passive turn-end hooks force one bounded follow-up.
...
A presence-gated sub-supervisor (`bin/fm-supervise-daemon.sh`) extends this for walk-away supervision: the `/afk` skill starts it through the tracked foreground helper `bin/fm-afk-start.sh`, after which the daemon manages the watcher and self-handles routine wakes in bash.
The away watcher's stale-pane and heartbeat paths are one-shot, enqueueing and exiting so the daemon classifies them, while its signal path runs the same conservative triage it runs when always-on and additionally forces immediate delivery of a persistent secondmate report.
Absorbing a provably-working crew's bare turn-end there removes that wake's process, IPC, disk, and queue overhead; it does not address upstream issue #1692 context amplification, which is a property of what a wake hands the model rather than of how many wakes are enqueued.
While away mode is active, the identity-matched daemon lock rather than the watcher lock is the proof of supervision for the turn-end guard, the session-start digest, and the pull-based guard, because the watcher lock is legitimately absent between wakes.
All three read that one daemon-liveness predicate from `bin/fm-wake-lib.sh`, so they cannot disagree about whether an away home is supervised at all: a missing daemon keeps the full alarm on every surface.
They differ only in what they do with a live daemon whose watcher beacon has gone stale, because a live daemon still owns away supervision there.
The turn-end guard allows the turn to end on the live daemon alone and never requires a fresh watcher beacon, while the pull-based guard and the session-start digest additionally report that state as degraded rather than healthy.
That degraded report names both halves of the actual coverage, because the daemon's marker rechecks and status-file catch-all scan keep running while pane-level staleness detection stops until the watcher is back.
The watcher and daemon share `bin/fm-classify-lib.sh` for captain-relevant status verbs, declared-external-wait vocabulary, keyed delivered-decision receipts, terminal-signal dedupe, and status-scan primitives.
The daemon's digest classifier applies the identical delivered-decision policy: a resolved echo with a matching receipt is self-handled and its receipt consumed, while an unmatched or bundled resolved event escalates in the digest.
...
A digest whose submit cannot be confirmed may already have been accepted, so its durable identity in `state/.subsuper-digest-inflight` suppresses every automatic retype of that logical digest across flushes, shutdown, and restart, and raises the same alarm exactly once; the invariant and its per-digest scope are owned by [herdr-backend.md](herdr-backend.md#current-transport-behavior).
...
Remote lifecycle keeps `fm-control.sh` as its only semantic plane: the parent transports the exact-id verb through `fm-on`, and the remote host verifies the postcondition against its host-local endpoint record without routing the command through marked chat.
...
PR-based task merges go through `bin/fm-pr-merge.sh`.
For a live task it records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before calling `gh-axi pr merge`.
For a PR-bearing ship task already cleaned up successfully, teardown's private exact-PR receipt permits only that same PR and is retired after a successful merge.
...
The same pass builds a bounded inventory of important open work records the current session already knows are missing or stale, then files or corrects them through their existing owners without crossing secondmate ownership.
It never enumerates other records to expand that inventory and never represents its session-scoped persistence as a reconciliation of durable state against repository, forge, or fleet reality.
