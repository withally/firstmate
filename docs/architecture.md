...
Actionable wakes include captain-relevant status signals or keyed resolutions that close currently open decisions, authenticated check output such as PR merge polling or a Relay mention, the built-in fseventsd footprint early warning, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Once that row reaches `FM_SECONDMATE_WAKE_STALL_SECS`, the primary appends one keyed `check` wake only when the secondmate home's own `state/.last-watcher-beat` has no usable mtime younger than the same threshold, or when the row age reaches three times the threshold regardless of the beat.
The setting's default and invalid-value fallback are documented in [Configuration](configuration.md#environment-variables); parent receipts and queued-key deduplication suppress repeats for the same row across watcher and handling crashes, while empty queues, younger rows, and ordinarily aged rows beside a fresh beat remain silent.
...
`tests/fm-wake-queue.test.sh` pins the default bound, beat-aware notification, three-times-age fallback, idempotence, quiet-queue, and byte-for-byte foreign-row preservation guarantees.
...
Attended signal triage applies the shared captain-relevant predicate to newly appended status lines, absorbing nonterminal appends and bare turn-ended signals for ordinary crews and secondmates alike; a keyed `resolved` wakes only when it closes a currently open fold key, while a non-closing resolution remains for `UNREAD STATUS`.
The attended signal classifier owns a separate per-task `.signal-open-keys` snapshot keyed to the signal's seen byte boundary, rebuilding from that snapshot boundary to the captured endpoint when needed, so it never aliases the drain's forward-only open-decision cursor.
The stale path is the single owner of bounded stopped-crew detection when the latest status remains nonterminal.
...
A queued signal annotation prints every status line still unread at that cursor, while the fleet-wide UNREAD STATUS section prints each still-unread nonterminal status line once, except a keyed resolution that closes an open key because its signal annotation is the single presentation, even on an empty-queue drain because these events do not belong to the OPEN DECISIONS fold.
...
The explicit resolution is written by the actor that answers, not the busy worker: `fm-send`'s `--resolve-key` appends the closing `resolved` line to this home's own copy of the ledger at answer time for crewmates and parent-level local or remote secondmate decisions; remote mate escalations reach that copy through parent-replies ingest, while Pi-mirrored secondmate child decisions use the mate-home ledger and close the parent fold through the mirror documented in [Pi supervision branch](pi-supervision-branch.md#two-stage-noise-filter).
...
[docs/supervision-protocols/pi.md](supervision-protocols/pi.md) owns MAIN's captain-visible response and captain and firstmate-action outcome handling; every other harness keeps the wake-to-main path unchanged.
...
`bin/fm-brief.sh` places the shared operational-input silence rule once under the status protocol in every generated ship, scout, and secondmate worker scaffold, with the wording owned by `bin/fm-operational-input.sh`.
...
The daemon's catch-all considers only status bytes beyond that library's shared presentation cursor, so attended history settled before away-mode entry stays settled while later appends remain eligible.
...
The always-on watcher uses that library's absorb classification on first-sighting stale panes before status-log terminality is trusted, while the daemon maintains distinct wedge and declared-wait recheck cadences.
...
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`: tmux keeps the same submit core used by the tmux send backend, while Herdr uses the harness-aware confirmation policy in [Current transport behavior](herdr-backend.md#current-transport-behavior).
The retries-exhausted queued-Enter decision is owned by `fm_composer_queued_enter_verdict` in `bin/fm-composer-lib.sh`; tmux and Herdr provide only their backend-specific delivery signals.
...
`fm-send.sh` delivers every remote text steer and ordinary local text steer as a durable steering-inbox record plus a best-effort constant doorbell line (`bin/fm-task-inbox-lib.sh`), which carries the shared silent-reply suffix from `bin/fm-operational-input.sh`.
...
## Delivery modes and merge authority are explicit per task
...
Each task's mode and merge-authority tier are firstmate's decision at intake.
The mode and tier are passed explicitly to `bin/fm-brief.sh`, while `bin/fm-spawn.sh` also receives compatibility `yolo`; each command refuses a concrete brief mismatch.
A ship brief records its mode and, when supplied, its authority contract, while a legacy brief without that line lets spawn derive the tier from `yolo`.
Scout promotion follows the fresh authority resolution in [configuration.md](configuration.md#project-registry-dataprojectsmd), so it does not preserve a stale seeded tier.
`data/projects.md` records each project's standing delivery and merge-authority posture, including the conditional `no-mistakes-prod-only` policy; a ship spawn that drops below the registered rigor prints a deviation notice and continues.
The registry schema and legacy mapping live in [configuration.md](configuration.md#project-registry-dataprojectsmd), and `bin/fm-project-mode.sh` is the one parser used by charter scaffolding, promotion, home seeding, fleet sync, and the spawn's advisory registry-deviation check.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=`, any available `pr_head=`, and the checked head used for green proof as `pr_checks_head=` through `bin/fm-pr-check.sh` before calling the forge CLI.
...
Before any provider merge, the configured authority and its landing proof are enforced by `bin/fm-merge-authority-lib.sh`; GitHub also requires live green checks at the current head.
...
A confirmed merge leaves a durable role-routed outcome instead of living only in the merging agent's memory, and [`bin/fm-merge-outcome-lib.sh`](../bin/fm-merge-outcome-lib.sh)'s header owns its destination, parent-binding validation, shape, identity, normal-case deduplication, and at-least-once recovery.
