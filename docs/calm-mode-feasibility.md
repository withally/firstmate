...
| `fm_branch_report` | Branch-session custom tool supplied directly to `createAgentSession` | It runs only in the headless supervision session and has no main-session `ToolExecutionComponent`; successful execution writes the outcome store, while [the branch contract](pi-supervision-branch.md#two-stage-noise-filter) owns delivery semantics, so the tool cannot emit a dump-shaped row in the captain's transcript. |
