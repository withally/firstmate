...
| `synthetic-assistant` | Exact no-action acknowledgement belonging to an agent run whose every Firstmate input is a canonical operational user row | The scoped rule is owned by [`calm.md`](calm.md); the API-probed assistant layout adapter supplies zero-height live and replay presentation without changing the message, and rows replayed through a transcript rebuild stay scored per row even when the rebuild happens inside a run. |
