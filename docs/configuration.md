...
FM_PAUSE_RESURFACE_SECS=           # opt-in recheck cadence; unset/empty means an absorbed idle declared external wait (or captain-held transfer, or already-delivered terminal status) stays silent in the watcher and away-mode daemon; set to seconds to re-surface one recheck per window per cadence
