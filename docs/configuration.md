...
Queued `check` records are coalesced only when their kind, durable key, and payload are identical, both when a publisher checks the durable queue and when a drain prepares presentation; a changed payload remains a separate delivery even when its key, including a source-and-sequence identity, is unchanged.
