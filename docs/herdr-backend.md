...
Pi + Herdr away mode is unverified and fails closed through the compatibility contract owned by `bin/fm-supervise-daemon.sh`; `/afk` must enter through `fm-afk-start.sh`.
Turning Calm off is not a supported workaround because Pi editor classification and ambiguous submit acknowledgement are not verified together in this fork.
