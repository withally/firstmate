...
A Pi literal that does not remain visibly pending in the current composer during a bounded pre-Enter settle reports `text-not-typed` without sending Enter.
A long Pi literal is compared with the composer body after whitespace normalization and bounded polling; a provably shorter normalized body reports `text-truncated` without sending Enter, while any other unproven body remains conservative and can produce exit 3 after Enter.
A Pi message still visibly present after the final wait reports `not-submitted` when it is a proven pending composer state; a long message without its required integrity proof is `pending-unproven` and reports exit 3, as does a cleared or unreadable surface without either positive signal.
...
For other non-Claude targets, a pane already mid-turn cannot borrow a rendered-footer transition as proof of this delivery; after retries, only proven pending text plus native `working` can establish that its Enter was accepted and queued.
