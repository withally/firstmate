...
For a known Claude target, Herdr's native `working` signal is diagnostic only because a tracked background shell can keep it set after the foreground turn ends.
