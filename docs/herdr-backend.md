...
A Pi literal whose composer is proven empty during a bounded pre-Enter settle reports `text-not-typed` without sending Enter.
Long Pi literals use the same bounded structural observation as short literals, without reconstructing the composer body from screen rows.
A valid non-empty Pi pair must also contain bounded leading and trailing anchors from the current literal, plus one bounded interior anchor for multiline input; a mismatch reports `text-not-typed` before Enter instead of treating another retained draft as the current message.
