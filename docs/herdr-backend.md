...
A Pi literal that never appears in the composer during a bounded pre-Enter settle reports `text-not-typed` without sending Enter, which makes a dropped long `pane send-text` deterministic.
A long Pi literal must match the complete composer body during that pre-Enter settle; a prefix-only or mismatched body reports `text-truncated` without sending Enter, while a later busy queue preview may remain prefix-only after the full preflight proof.
A Pi message still visibly present after the final wait reports `not-submitted`; a cleared or unreadable surface without either positive signal reports the exit-3 unconfirmed state.
