...
`pane_is_busy` therefore requires the rendered Claude active-turn signature, such as `esc to interrupt` or a spinner with elapsed time, on the current final nonblank footer row after the shared classifier proves its composer context before declaring the pane busy.
