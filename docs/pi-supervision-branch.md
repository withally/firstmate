...
Captain-facing branch outcomes persist as exact, sequence-keyed visible transcript entries and then open one sequence-keyed processing turn on main, which stays open until main acknowledges that sequence.
...
- The branch itself: `.pi/extensions/fm-branch-supervision.ts` creates the branch session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  Every accepted path that cannot reach a working branch rejects its settlement to the watcher, which retains delivery ownership and routes the wake through the temporary Pi self-delivery containment owned by [`watcher-continuity.md`](watcher-continuity.md#temporary-pi-self-delivery-containment); a broken branch declines later offers so they take that path directly.
...
- Outcome store: `bin/fm-branch-outcome.sh`; its header owns the append-only format, read cursor, and bounded per-task status-coverage indexes.
  ...
  A legacy `firstmate-action` row with neither `wake_seq` nor an action marker remains visible to startup replay.
...
Stage two is the branch's three-way verdict on each handled event, reported through its `fm_branch_report` tool: `routine` keeps the existing custom-message path without a follow-up turn, `captain` appends a versioned `fm-branch-visible-outcome` custom session entry, and `firstmate-action` records an already-authorized downstream action for a hidden handoff to main.
...
Reconciliation runs at session start when that generation already owns the fleet lock and at the first post-lock `turn_end`, so a cold start that acquires the lock through the startup digest still delivers stored captain outcomes and retries eligible wake-linked action handoffs without waiting for another wake.
...
The branch prompt's "Verdict: routine, captain, or firstmate-action" section owns the verdict criteria, including how requested work's finished results and its mere progress updates are classified; unsolicited routine outcomes remain routine sailboat notes, unchanged fleet reviews remain silent, and doubt escalates.
...
A review with an already-authorized downstream action reports verdict `firstmate-action` and uses the hidden handoff to main.
