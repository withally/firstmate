...
A mate's `bin/fm-send.sh --resolve-key <child-key> '<answer>'` writes `resolved` in the mate home ledger, and the parent fold closes when the mirror carries that resolution under its projected key.
