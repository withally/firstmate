...
- The branch itself: `.pi/extensions/fm-branch-supervision.ts` creates and reopens the persistent branch session, coalesces wakes, mirrors dialog, and delivers outcomes.
  ...
  A current-generation path that cannot reach a working branch falls back to delivering the wake to main - a broken branch degrades to today's behavior, never to a lost wake.
  A continuation from a replaced generation is discarded instead, while its accepted rows remain durable until the drain acknowledgement.
...
- Outcome store: `bin/fm-branch-outcome.sh`; its header owns the append-only format and the read cursor.
  ...
  Firstmate-action reports persist pending handoff state and a per-wake marker before the outcome cursor can advance; `wake_seq`, rather than the outcome `seq`, is the idempotency authority, so a re-presented wake recovers the pending row instead of opening a duplicate handoff.
  A failed or crash-interrupted hidden delivery leaves the outcome unread for recovery to replay as the same operational `firstmate-action` envelope, not as raw JSON.
...
- Autonomy: supervision is default-on for every task once a Pi primary session owns the fleet lock (docs/configuration.md "Pi supervision branch"); no captain grant file is required.
  ...
  Away mode and a broken current-generation branch keep today's wake-to-main behavior.
...
After the wake is acknowledged and the branch's eligible-row grant and task leases are released, it sends one hidden delivery message that triggers exactly one follow-up turn for MAIN to perform that action immediately and then report the result.
...
An identical same-text `stale:` repeat for that window opens no immediate branch turn while that text remains in flight, but it schedules one normal pre-drain recheck after the current handling settles so a row outside the current claim is not stranded.
