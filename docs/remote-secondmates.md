...
The project's registered delivery mode and merge-authority posture still come from this machine's `data/projects.md`, so an unregistered or `local-only` project is refused rather than provisioned.
...
If delivery remains unconfirmed, only the exact `FM_PENDING_REPLY_EXISTING_CORR=<id>` resend command printed by `fm-send` is safe to run later because it preserves the persisted attempt body and lets the remote enqueue deduplicate onto the same record; a plain rerun mints a different correlation and is not idempotent.
...
`--force` remains the explicit discard path and follows hard rule 3 in [`AGENTS.md`](../AGENTS.md), including explicit authorization for the named contents.
