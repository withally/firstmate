...
| `fm-lavish-audit.sh`     | Audit Lavish registry ownership, freeze candidates, and apply bounded verified endings |
| `fm-lavish-session.sh`   | Record Lavish ownership, end ephemeral sessions, and safe-park durable reviews     |
...
| `fm-project-mode.sh`     | Parse and resolve a project's registered delivery and merge-authority posture from `data/projects.md` for charter scaffolding, promotion, home seeding, fleet sync, and spawn deviation checks |
...
| `fm-procevent-lavish.sh` | Arm, poll, acknowledge, classify, and retire Lavish process-event sources          |
...
| `fm-lavish-lib.sh`       | Shared Lavish state-path resolution and lifecycle helper primitives                |
...
| `fm-upstream-urgent-check.sh` | Report upstream commits added after the latest adopted sync base whose subject or body carries an urgent keyword; arms and disarms its own watcher check |
