...
The branch's authoritative three-verdict outcome-delivery contract lives in [docs/pi-supervision-branch.md](../pi-supervision-branch.md#two-stage-noise-filter).
At this protocol boundary, remember that `routine` outcomes stay store-only, `captain` opens one turn to surface a genuine captain call without fleet replay, and `firstmate-action` opens one turn to perform the authorized downstream action before reporting its result.
