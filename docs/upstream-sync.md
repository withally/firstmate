...
9. Re-apply only the kept behaviors on top of the recorded upstream base.
   ...
   The beat-aware secondmate wake-loop stall detector - a fresh foreign watcher beat suppresses the ordinary threshold alert, while a row older than three thresholds always alerts - is a kept fork patch and must be re-applied when upstream lacks it.
