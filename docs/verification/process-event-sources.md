...
| unchanged-result re-announcement and exact check presentation | after that first proactive wake, an immutable still-unhandled result keeps being re-announced onto the durable queue but never wakes the watcher again; the exact kind/key/payload coalescing and changed-payload behavior follows the [runner contract](../configuration.md#process-to-event-sources-stateprocevent); once existing records receive the drain's post-handling acknowledgement and the source result is acknowledged, it is neither re-announced nor reported |
