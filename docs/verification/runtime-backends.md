...
Two refresh attempts on 2026-09-07 with Herdr 0.8.2 and Claude Code 2.1.263 failed because the foreground `/afk` turn kept working instead of settling into the guard's native-working, rendered-idle precondition; no refreshed pass is claimed, and the original assertion remains intact.
The separate live submit-confirmation guard passed on those installed versions, while the portable suites carry the red/green evidence for the historical away-mode state shape.
