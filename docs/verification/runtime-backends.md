...
| Native state | `herdr agent get <pane>` | Working and done transitions were visible on some harnesses; live Claude Code 2.1.236 on Herdr 0.8.0 kept `agent_status=idle` for an entire landed turn, including a multi-second tool call, so non-Claude submit confirmation can fall through to the shared composer verdict. For an away-mode Claude target, native `working` is diagnostic only and the current rendered footer plus composer proof decide injection and submit confirmation. Native `busy` remains positive activity evidence for the semantic worker-state path, while native `idle` cannot close a turn. |
...
The portable regressions in `tests/fm-backend-herdr.test.sh` and `tests/fm-composer-lib.test.sh` pin the split verdicts: non-Claude native idle plus a cleared composer is delivery, proven pending plus idle is a swallow, and proven pending plus a generating busy signal is a queued Enter; a known Claude target instead requires a current-footer idle-to-busy transition or a cleared composer, and stale native `working` or a pre-existing busy footer cannot confirm its Enter.
