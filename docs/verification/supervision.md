...
That inertness result did not establish that `GROK_AGENT` reaches a Grok hook process, and grok 1.0.0 demonstrated that it does not, so the marker set now includes `GROK_HOOK_EVENT`.
`tests/fm-turnend-guard.test.sh` pins every tracked `.claude/settings.json` hook command against a Grok hook environment so this contract is deterministic rather than only an opt-in live result.
...
The same suite also covers decision-only recovery that resurfaces the folded open-decision set without inventing a queue row, and process-event delivery taking first refusal ahead of the generic `check: rearm-resurface`.
`tests/fm-wake-queue.test.sh` owns the durable-queue half: interrupted presentation replaying exactly once, a stale recovery generation being refused without consuming newer work, current-format upgrade adoption, and malformed-row quarantine that still finalizes through an empty-queue acknowledgement.
