...
A failed main-turn delivery never cancels continuity restoration.
Pi same-process session replacement follows the generation-owner contract in `.pi/extensions/fm-primary-pi-watch.ts`: an owning `session_start` arms the replacement generation without waiting for a model turn, and a state-scoped replacement handoff carries every actionable close whose delivery overlapped `session_shutdown`, including a main wake waiting for `agent_settled` or not yet consumed, branch handling, and a retiring child that reports after the bounded shutdown wait.
Pi's watcher never sends wake work through the streaming follow-up queue.
A follow-up accepted after Pi's final queued-message check but before `agent_settled` can otherwise remain stranded until an unrelated external prompt starts another run.
The extension tracks `agent_start` through `agent_settled`, waits for the settled boundary, submits the wake as a fresh turn, and counts it consumed at `before_agent_start` or the equivalent user `message_start`.
...
If a branch offer loses the claim race to main, it rejects its settlement so the watcher retains the actionable close until Pi starts its fresh main turn.
...
The same suite reproduces Pi's settled-boundary race and proves that the wake waits for `agent_settled` and starts as a fresh turn instead of entering the strandable follow-up queue.
...
`FM_PI_HERDR_WAKE_LIVE=1 tests/fm-pi-herdr-wake-live-e2e.test.sh` provisions a named Herdr lab, appends a real status wake, and proves that Pi drains and acknowledges its durable row without any parent doorbell.
