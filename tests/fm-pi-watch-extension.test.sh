...
test_pi_cleanup_failure_status_preserves_unrelated_notice() {
  local repo home plugin fail stop out status
  repo="$TMP_ROOT/pi-cleanup-failure-status-owner-root"
  home="$TMP_ROOT/pi-cleanup-failure-status-owner-home"
  fail="$TMP_ROOT/pi-cleanup-failure-status-owner.fail"
  stop="$TMP_ROOT/pi-cleanup-failure-status-owner.stop"
  mkdir -p "$repo/bin" "$home/state/extensions/pi-primary-watch" "$home/config"
  install_pi_watch_extension_fixture "$repo"
  plugin="$repo/.pi/extensions/fm-primary-pi-watch.ts"
  cat > "$home/state/extensions/pi-primary-watch/session-replacement-actionable.json" <<'JSON'
{"version":2,"pending":[{"version":1,"token":"111-222-1","message":"signal: cleanup status owner outcome","predecessorArmPid":"0"}]}
JSON
  cat > "$repo/bin/fm-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = --handling-delivered ]; then
...
printf 'watcher: started pid=%s recovery-generation=status-owner-fixture\n' "$$"
trap 'exit 0' TERM INT
while [ ! -e "$FM_FAIL_FILE" ] && [ ! -e "$FM_STOP_FILE" ]; do sleep 0.02; done
if [ -e "$FM_FAIL_FILE" ]; then
  rm -f "$FM_FAIL_FILE"
  printf 'watcher: FAILED - unrelated watcher failure\n'
  exit 3
fi
SH
  chmod +x "$repo/bin/fm-watch-arm.sh"
  out=$(PLUGIN="$plugin" FM_HOME="$home" FM_ROOT_OVERRIDE="$repo" FM_FAIL_FILE="$fail" FM_STOP_FILE="$stop" FM_WATCH_REARM_RETRY_BASE_MS=10000 FM_WATCH_REARM_RETRY_MAX_MS=10000 node --input-type=module 2>&1 <<'EOF'
...
writeFileSync(
  ...
  JSON.stringify({
    version: 2,
    pending: [{ version: 1, token: "111-222-1", message: "signal: cleanup status owner outcome", predecessorArmPid: "0" }],
  }) + "\n",
);
await handlers.get("agent_settled")?.({}, idle);
await waitFor(() => !existsSync(handoff), "cleanup of the original row");
const failureEvents = statuses.filter(({ key }) => key === "firstmate-watcher-failure");
const latestFailure = failureEvents[failureEvents.length - 1];
if (!latestFailure || !String(latestFailure.value).includes("unrelated watcher failure")) {
  throw new Error(`cleanup of row A cleared unrelated failure B: ${JSON.stringify(failureEvents)}`);
}
if (!String(latestFailure.value).includes("parent doorbell owns recovery")) {
  throw new Error(`unrelated failure lost parent-doorbell recovery: ${latestFailure.value}`);
}
if (sends.length !== 1) throw new Error(`unrelated failure created another turn: ${JSON.stringify(sends)}`);
writeFileSync(process.env.FM_STOP_FILE, "stop\n");
await handlers.get("session_shutdown")?.({ type: "session_shutdown", reason: "quit" }, idle);
process.exit(0);
EOF
  ...
  status=$?
  expect_code 0 "$status" "Pi cleanup must preserve an unrelated failure status owner: $out"
  [ -z "$out" ] || fail "Pi cleanup failure status-owner test printed output: $out"
  pass "Pi cleanup preserves an unrelated failure status owner"
}
