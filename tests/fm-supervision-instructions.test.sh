...
test_pi_snippet_uses_effective_extension_path() {
  ...
  assert_contains "$out" "Treat the wake behind either opened turn as already acknowledged; any legacy rendered branch note is historical and already handled - do not re-drain or re-handle its event." "pi snippet lost wake acknowledgement"
  assert_contains "$out" "For \`captain\`, MAIN applies judgment about how to surface the captain-only result or question without taking another fleet action from that delivery." "pi snippet lost captain-only treatment"
  assert_contains "$out" "For \`firstmate-action\`, wake ownership does not complete the downstream workflow: perform the authorized action named by the envelope now, then report its result." "pi snippet lost firstmate-action treatment"
