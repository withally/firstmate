...
test_pi_snippet_uses_effective_extension_path() {
  ...
  assert_contains "$out" "Treat an opened captain-facing turn as already handled; any legacy rendered branch note is historical and already handled - do not re-drain or re-handle its event." "pi snippet lost captain-turn event ownership"
  assert_contains "$out" "Separately, MAIN applies judgment about whether and how to surface, summarize, reference, or incorporate the captain outcome in the captain conversation; event ownership does not decide the conversational treatment." "pi snippet imposed a mechanical captain-outcome treatment"
