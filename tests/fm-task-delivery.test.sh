...
STUB
  ...
  for mode in no-mistakes direct-PR local-only; do
    ...
    assert_grep "The shared status protocol rules carry over unchanged, but the mode-specific working-phase rule below supersedes the scout wording." "$payload" \
    ...
    case "$mode" in
      ...
      local-only)
        ...
        assert_no_grep 'implementation committed and validation started, or PR opened' "$payload" \
          "local-only: promoted worker still permits a PR working phase" ;;
    esac
  ...
  assert_grep "The instruction inbox and its acknowledgement; the escalation rules, including ask-user; and every safety rule carry over unchanged" "$payload" \
