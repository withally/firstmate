...
test_path_skew_is_reported_from_every_copy() {
  ...
  [ "$(wc -l < "$out")" -eq 1 ] || fail "the report must be exactly one line for the wake record"
...
test_one_broken_pattern_does_not_blind_the_rest_of_the_sweep() {
  ...
  [ "$(wc -l < "$out")" -eq 1 ] || fail "the report must stay exactly one line"
...
test_an_overlong_report_says_it_was_cut() {
  ...
  [ "$(wc -l < "$out")" -eq 1 ] || fail "the cut report must still be exactly one line"
