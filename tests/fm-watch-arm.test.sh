...
test_rearm_resurfaces_durable_queue_and_remote_open_decision() {
  ...
  # Recovery startup is a real process boundary, so use the existing bounded
  # wait instead of assuming the host schedules it within 250 milliseconds.
  wait_for_exit "$ARM_PID" 120
  status=$?
  [ "$status" -eq 0 ] \
    || fail "re-arm did not surface durable wakes and the still-open remote decision: $(cat "$armout")"
